...
type MultiVal a = Val a
type InterventionPointKey m a = HKey T (Delta m a)

getCounterfactual :: Val a -> a
getCounterfactual (Val (ns, f)) = f (constWorld True ns)
...
--     => HKey T (Delta m Bool) -> Caus m (Val Bool, Val Bool, Val Bool)
...
bobInferenceCode :: IO ()
bobInferenceCode = do -- IO
    -- Generate the intervention point key
    fuelKey <- createKey
    let intervenedConditionedModel = do -- Caus
...
bobCarModelIntervenedAndConditioned :: MonadMeasure m
    => HKey T (Delta m Bool) -> Caus m (Val Bool, Val Bool, Val Bool)

bobCarModelIntervenedAndConditioned fuelKey = do -- Caus
...
bobCarModelCF = do -- IO
    ...
    let distCFCarStarts = do -- Caus
            (_, _, carStarts) <- distModel;
            let (ns, val) = getVal carStarts;
...
aliceCarModel :: MonadDistribution m
    => HKey T (Delta m Bool) -> Caus m (Val Bool, Val Bool)
...
aliceCarModelIntervenedAndConditioned :: MonadMeasure m
    => HKey T (Delta m Bool) -> Caus m (Val Bool, Val Bool)

aliceCarModelIntervenedAndConditioned fuelKey = do -- Caus
...
modelOperation1 :: Caus m (Val a) -> HKey T (Delta m Int) -> Caus m (Val a)
modelOperation1 model key =
    let newModel1 = do_ key (Value 5) "branch1" model in
    let newModel2 = do_ key (Value 10) "branch2" newModel1 in
    newModel2


modelOperation2 :: Caus m (Val a) -> HKey T (Delta m Int) -> HKey T (Delta m Int) -> Caus m (Val a)
modelOperation2 model key1 key2 =
    let newModel1 = do_ key1 (Value 5) "branch" model in
    let newModel2 = do_ key2 (Value 10) "branch" newModel1 in
    newModel2

main_models_1 :: IO ()
main_models_1 = do
    key <- createKey;
    let model :: Caus Identity (Val Int) = new_ key (pure 1)
...
main_models_2 = do
    key1 <- createKey;
    key2 <- createKey;
    let model :: Caus Identity (Val (Int, Int)) = do
         a <- new_ key1 (pure 1)
         b <- new_ key2 (pure 1)
         return $ (,) <$> a <*> b
